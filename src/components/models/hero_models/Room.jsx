...
export function Room(props) {
  ...
  const curtainMaterial = useMemo(() => new THREE.MeshPhongMaterial({
    color: "#d90429",
  }), []);

  const bodyMaterial = useMemo(() => new THREE.MeshPhongMaterial({
    map: matcapTexture,
  }), [matcapTexture]);

  const tableMaterial = useMemo(() => new THREE.MeshPhongMaterial({
    color: "#582f0e",
  }), []);

  const radiatorMaterial = useMemo(() => new THREE.MeshPhongMaterial({
    color: "#fff",
  }), []);

  const compMaterial = useMemo(() => new THREE.MeshStandardMaterial({
    color: "#fff",
  }), []);

  const pillowMaterial = useMemo(() => new THREE.MeshPhongMaterial({
    color: "#8338ec",
  }), []);

  const chairMaterial = useMemo(() => new THREE.MeshPhongMaterial({
    color: "#000",
  }), []);
